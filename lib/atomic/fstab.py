...
def update_fstab_home(path_str: str, new_home_subvol: str) -> bool:
    ...
    Steps:
      1. Back up original to .home-bak
      2. Parse all lines, modify only mountpoint=="/home" entries
    ...
    # Validate subvol name: alphanumeric, hyphens, underscores, dots
    if not re.match(r'^[a-zA-Z0-9][a-zA-Z0-9._-]*$', new_norm):
        print(
            f"ERROR: Invalid home subvolume name: {new_norm}",
            file=sys.stderr,
        )
        return False
    ...
    # Find /home mount entries (exact match on mountpoint)
    ...
    if not home_entries:
        ...
        print(
            "WARN: Environment will share the host /home directory",
            file=sys.stderr,
        )
        backup.unlink(missing_ok=True)
        return False

    # Check that at least one has subvol=
    has_subvol = any(
        "subvol=" in e.options
    ...
    if not has_subvol:
        print(
            "WARN: /home entry exists but has no subvol= option",
            file=sys.stderr,
        )
        print(
            "WARN: Environment will share the host /home directory",
    ...
    if updated == 0:
        # subvol= already had the right value
    ...
    text = path.read_text()
    if (
        f"subvol=/{new_norm}" not in text
        and f"subvol={new_norm}" not in text
    ):
        print(
            "ERROR: /home fstab verification failed, restoring backup",
            file=sys.stderr,
        )
        shutil.copy2(backup, path)
        backup.unlink(missing_ok=True)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(
            f"       {sys.argv[0]} home FSTAB_PATH NEW_HOME_SUBVOL",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if sys.argv[1] == "home":
        if len(sys.argv) != 4:
            print(
                f"Usage: {sys.argv[0]} home FSTAB_PATH NEW_HOME_SUBVOL",
                file=sys.stderr,
            )
            sys.exit(1)
